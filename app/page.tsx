...
export default function Home() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      if (isSigningOut) {
        console.log("[v0] Ignoring auth state change during sign out")
        return
      }
      ...
      if (firebaseUser && firebaseUser.emailVerified) {
        ...
        try {
          ...
          console.log("[v0] Cart load failed, starting with empty cart")
          setCart([])
        }
  ...
  return (
    <div className="flex min-h-screen bg-background">
      ...
      <main className="flex-1 w-full lg:ml-0 overflow-y-auto">
        <div className="container mx-auto px-4 py-20 lg:py-8 lg:px-6 xl:px-8 max-w-7xl">
                ...
                {authLoading ? (
                  ...
                  <div className="flex items-center gap-2 sm:gap-3">
                    <Button
                      variant="secondary"
                      size="sm"
                      className="px-3 sm:px-4 min-w-fit"
                      onClick={() => setShowProfileDialog(true)}
                    >
                      <User className="h-4 w-4 flex-shrink-0" />
                      <span className="ml-2 text-sm font-medium max-w-[100px] sm:max-w-[150px] truncate">
                        {user.name}
                      </span>
                    </Button>
                    <Button
                      variant="outline"
                      size="sm"
                      className="bg-transparent text-sm px-4 whitespace-nowrap min-w-[90px]"
                  ...
                  <Button
                    variant="default"
                    size="sm"
                    className="text-sm px-6 whitespace-nowrap shadow-sm"
