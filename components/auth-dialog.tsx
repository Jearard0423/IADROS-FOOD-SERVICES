...
export function AuthDialog({ open, onOpenChange, onAuthSuccess }: AuthDialogProps) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (error.code === "auth/unauthorized-domain") {
        errorMessage =
          "⚠️ OAuth sign-in only works on the deployed website. To use Google sign-in: 1) Deploy this app using the Publish button, 2) Add your deployment domain to Firebase Console > Authentication > Settings > Authorized domains. For now, please use email/password authentication."
  ...
  const handleFacebookSignIn = async () => {
    ...
    try {
      ...
      if (error.code === "auth/unauthorized-domain") {
        errorMessage =
          "⚠️ OAuth sign-in only works on the deployed website. To use Facebook sign-in: 1) Deploy this app using the Publish button, 2) Add your deployment domain to Firebase Console > Authentication > Settings > Authorized domains. For now, please use email/password authentication."
      } else if (error.code === "auth/popup-closed-by-user") {
        errorMessage = "Sign-in was cancelled. Please try again."
      } else if (error.code === "auth/account-exists-with-different-credential") {
        errorMessage = "An account already exists with this email using a different sign-in method."
          ...
          <TabsContent value="login" className="space-y-4 mt-4">
            <div className="space-y-3">
              <div className="text-xs text-muted-foreground text-center px-2">
                Google & Facebook sign-in work on the deployed site
              </div>
              <Button
                type="button"
                variant="outline"
